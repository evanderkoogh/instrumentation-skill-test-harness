...
Emit **all three OpenTelemetry signals — traces, metrics, and logs** — over OTLP to the endpoint above. Do not scope the engagement to traces alone: enable metric and log export/auto-instrumentation alongside tracing so all three signals reach Honeycomb.
...
- **Capture telemetry with a console/file exporter** (e.g. `OTEL_TRACES_EXPORTER=console`, `OTEL_METRICS_EXPORTER=console`, `OTEL_LOGS_EXPORTER=console`) and inspect it directly — confirm spans **and** metric datapoints **and** log records are produced. Do NOT rely on Honeycomb or depend on telemetry arriving there — the test harness runs the authoritative Honeycomb evaluation (its own traffic + queries) after you finish.
